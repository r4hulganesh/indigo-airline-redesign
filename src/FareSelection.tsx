...
export default function FareSelection({ onBack, onNext }: { onBack: () => void; onNext: () => void }) {
  ...
  const fares = {
    ...
    flexi: {
      ...
      basePrice: 24300,
      bluChipSavings: 1500,
      subtitle: '+₹1,500 for flexibility',
    ...
    upfront: {
      ...
      basePrice: 26400,
      bluChipSavings: 1500,
      subtitle: '+₹3,600 for premium comfort',
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center text-center mt-4 relative">
        <div className="flex items-center gap-4 text-white/60 hover:text-white cursor-pointer transition-colors absolute left-4 lg:left-8 top-0 lg:top-4 mt-2 lg:mt-0 z-10" onClick={onBack}>
          <ChevronLeft className="w-5 h-5" />
          <span className="hidden sm:inline font-medium">Back to flights</span>
        </div>

        <div className="flex items-center gap-3 mb-4">
          <div className="w-5 h-5 bg-white rounded flex items-center justify-center p-0.5 shrink-0">
             <img src="https://upload.wikimedia.org/wikipedia/commons/6/69/IndiGo_Airlines_logo.svg" alt="IndiGo" className="w-full h-full object-contain" />
          </div>
          <span className="text-sm font-semibold text-white/70">IndiGo &bull; 6E 892</span>
        </div>

        <div className="flex items-center justify-center gap-4 mb-2">
          <h2 className="text-4xl font-black tracking-tight text-white">10:30</h2>
          <ArrowRight className="w-6 h-6 text-white/40" />
          <h2 className="text-4xl font-black tracking-tight text-white">12:40</h2>
        </div>
        
        <div className="flex items-center justify-center gap-2 mb-2 font-medium text-white/70">
          <span>Kochi</span>
          <ArrowRight className="w-3.5 h-3.5 text-white/40" />
          <span>Dubai</span>
        </div>
        
        <p className="text-sm font-medium text-white/60 mb-8 mt-2">29 Apr • 1 Passenger • Economy</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8 relative">
        <FareCard 
          ...
          features={[
            ...
            { name: 'Change policy', value: 'Standard charges', isHighlight: false, icon: <AlertCircle className="w-4 h-4" /> },
            { name: 'Cancellation policy', value: 'Standard charges', isHighlight: false, icon: <AlertCircle className="w-4 h-4" /> },
            { name: 'Meal', value: 'No meal', isHighlight: false, icon: <Minus className="w-4 h-4 text-white/20" /> },
            { name: 'Seat', value: 'Standard seat', isHighlight: false, icon: <Armchair className="w-4 h-4" /> },
          ]}
        />
        ...
        <FareCard 
          ...
          features={[
            { name: 'Cabin baggage', value: '7kg', icon: <Briefcase className="w-4 h-4" /> },
            { name: 'Check-in baggage', value: '30kg', icon: <Briefcase className="w-4 h-4" /> },
            { name: 'Change policy', value: 'Partial charges', isHighlight: true, icon: <AlertCircle className="w-4 h-4 text-blue-400" /> },
            { name: 'Cancellation policy', value: 'Partial charges', isHighlight: true, icon: <AlertCircle className="w-4 h-4 text-blue-400" /> },
            { name: 'Meal', value: 'Complimentary meal', isHighlight: true, icon: <Utensils className="w-4 h-4 text-emerald-400" /> },
            { name: 'Seat', value: 'Complimentary standard', isHighlight: true, icon: <Armchair className="w-4 h-4 text-emerald-400" /> },
          ]}
        />
        ...
        <FareCard 
          ...
          features={[
            ...
            { name: 'Change policy', value: 'Zero change fee', isHighlight: true, icon: <CheckCircle2 className="w-4 h-4 text-emerald-400" /> },
            { name: 'Cancellation policy', value: 'Low cancellation charges', isHighlight: true, icon: <CheckCircle2 className="w-4 h-4 text-emerald-400" /> },
            { name: 'Meal', value: 'Complimentary meal', isHighlight: true, icon: <Utensils className="w-4 h-4 text-emerald-400" /> },
            { name: 'Seat', value: 'Front row seat', isHighlight: true, icon: <Armchair className="w-4 h-4 text-emerald-400" /> },
          ]}
        />
      </div>
      ...
      >
        <div className="glass-panel-dark border border-white/20 rounded-2xl px-6 py-4 flex items-center justify-between backdrop-blur-xl shadow-[0_-8px_30px_rgba(0,0,0,0.4)] pointer-events-auto w-full max-w-4xl relative overflow-hidden bg-[#1e0a3c]/95">
           ...
           <button onClick={onNext} className="bg-white text-[#001B94] hover:bg-gray-100 rounded-xl px-10 py-3.5 font-bold text-sm transition-transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 shadow-[0_4px_15px_rgba(255,255,255,0.1)] relative z-10 group/btn">
             Continue with {fares[selectedFare].title === 'Saver' ? 'Saver' : fares[selectedFare].title === 'Flexi Plus' ? 'Flexi' : 'UpFront'} - ₹{currentTotal.toLocaleString('en-IN')} <ArrowRight className="w-4 h-4 transition-transform group-hover/btn:translate-x-1" />
           </button>
        </div>
      </motion.div>
    </motion.div>
